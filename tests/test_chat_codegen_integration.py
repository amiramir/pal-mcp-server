"""Integration test for Chat tool code generation with Gemini 3.1 Pro.
...
``tests/gemini_cassettes/chat_codegen/gemini31_pro_calculator/mldev.json`` and run:
...
CASSETTE_PATH = CASSETTE_DIR / "gemini31_pro_calculator" / "mldev.json"
CASSETTE_REPLAY_ID = "chat_codegen/gemini31_pro_calculator/mldev"


@pytest.mark.asyncio
@pytest.mark.no_mock_provider
async def test_chat_codegen_saves_file(monkeypatch, tmp_path):
    """Ensure Gemini 3.1 Pro responses create pal_generated.code when code is emitted."""
    ...
    with monkeypatch.context() as m:
        ...
        m.setenv("GOOGLE_ALLOWED_MODELS", "gemini-3.1-pro")
        ...
        provider = ModelProviderRegistry.get_provider_for_model("gemini-3.1-pro")
